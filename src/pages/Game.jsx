...
function Game() {
    document.title = "Tabla de multiplicar · Juego de Glécio";
    ...
    const handleModalConfirm = () => {
        navigate("/play/sexualidad", { replace: true });
    };
    ...
    const setRankingScore = async (score) => {
        try {
            ...
            if (response.status_code === 201) {
                console.info("Puntos enviados al ranking");
            }
        } catch (error) {
            toast.error(
                error.message || "Error al enviar puntos al ranking",
                {
    ...
    return (
        ...
        >
            ...
            {/* main mantiene clases originales en desktop. En mobile usamos min-h-[100dvh] y flex-col para empujar los items al fondo */}
            <main className="pt-20 p-6 max-w-6xl mx-auto max-[580px]:pt-16 max-[580px]:p-4 max-[580px]:flex max-[580px]:flex-col max-[580px]:min-h-[100dvh]">
                ...
                {/* Contenido: mantiene justify-between en desktop y vira a columna (flex-1) en celular */}
                <div className="flex justify-between gap-10 max-lg:gap-5 max-[580px]:flex-col max-[580px]:flex-1 max-[580px]:gap-2">
                    {/* Multiplicacion y aciertos/errores */}
                    <div className="flex flex-col justify-between w-[480px] max-[580px]:w-full max-[580px]:flex-1">
                        {/* La cuenta: en mobile usamos flex-1 para crecer al maximo y empujar aciertos/errores hacia abajo */}
                    ...
                    <div className="max-w-[450px] max-[580px]:max-w-full max-[580px]:w-full max-[580px]:pb-6 max-[580px]:shrink-0">
                        <input
                            ...
                            value={userResponse}
                        />
                        {/* Se redujo auto-rows en mobile de 9vh a 8.5vh para evitar cortes */}
            ...
            <AnimatePresence mode="wait">
                {showModal && (
                    <Modal
                        ...
                        message="Todo tu progreso se perdera."
                        confirmText="Salir"
