...
function RankingList() {
    ...
    const fetchRankingNormal = async () => {
        ...
        try {
            ...
            toast.error(
                error.message ||
                    "Error al obtener datos del ranking. Intenta de nuevo mas tarde.",
                {
    ...
    const updateRanking = async () => {
        ...
        try {
            ...
            toast.error(
                error.message ||
                    "Error al actualizar el ranking. Intenta de nuevo mas tarde",
                { className: "bg-surface" },
            );
        } finally {
            setIsRankingUpdating(false);
    ...
    const handleResetRanking = async () => {
        ...
        try {
            ...
            toast.error(
                error.message ||
                    "Error al limpiar el ranking. Intenta de nuevo mas tarde",
                { className: "bg-surface" },
            );
        } finally {
            setShowModal(false);
    ...
    return (
        <>
            ...
            <div className="flex flex-col gap-3 p-8 max-md:pt-20 md:p-10 md:border rounded-2xl max-sm:w-screen max-sm:h-screen md:shadow-sm min-w-[350px] lg:min-w-[380px] md:w-1/2 text-darkGray border-borderColor bg-surface shadow-md lg:w-1/3">
                ...
                {!userInfo.isAdmin && (
                    <p className="text-base">
                        Mira a los mejores jugadores compitiendo para alcanzar
                        las mayores puntuaciones en el juego de multiplicacion.
                    </p>
                )}
            ...
            <AnimatePresence mode="wait">
                {showModal && (
                    <Modal
                        title="Seguro que deseas reiniciar el ranking?"
                        message="Todos los datos del ranking se eliminaran."
